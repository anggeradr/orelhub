...
local TABS      = {"Buy Hero","Auto Farm","Best Hero","Settings"}
local TAB_ICONS = {"🛒","⚔️","⭐","⚙️"}
...
makeLabel(fusePage,"⭐  Auto Equip Best Hero",14,Color3.fromRGB(255,215,0),1)
makeSep(fusePage,2)
makeLabel(fusePage,"Otomatis equip hero terbaik (rank tertinggi) ke semua slot base.",11,Color3.fromRGB(180,170,130),3)
makeSep(fusePage,4)

-- Info card
local equipCard=Instance.new("Frame",fusePage)
equipCard.Size=UDim2.new(1,0,0,54); equipCard.BackgroundColor3=Color3.fromRGB(22,20,10)
equipCard.BorderSizePixel=0; equipCard.ZIndex=5; equipCard.LayoutOrder=5
Instance.new("UICorner",equipCard).CornerRadius=UDim.new(0,8)
Instance.new("UIStroke",equipCard).Color=Color3.fromRGB(180,140,0)
local equipTxt=Instance.new("TextLabel",equipCard)
equipTxt.Size=UDim2.new(1,-16,1,0); equipTxt.Position=UDim2.new(0,8,0,0)
equipTxt.BackgroundTransparency=1; equipTxt.TextWrapped=true
equipTxt.Text="⭐  Klik tombol di bawah untuk equip hero terbaik ke semua slot."
equipTxt.TextColor3=Color3.fromRGB(255,215,0); equipTxt.TextSize=11
equipTxt.Font=Enum.Font.Gotham; equipTxt.TextXAlignment=Enum.TextXAlignment.Left; equipTxt.ZIndex=6

makeSep(fusePage,6)

-- Equip once button
local equipOnceBtn=makeBtn(fusePage,"⭐  Equip Best Hero Sekarang",Color3.fromRGB(160,120,0),7)

local function doEquipBest()
    if not GetDataRemote or not PlaceHeroRemote then
        setStatus("❌ Remote tidak ditemukan",Color3.fromRGB(255,80,80)); return
    end
    local ok, data = pcall(function() return GetDataRemote:InvokeServer() end)
    if not ok or not data or not data.Heroes then
        setStatus("❌ Gagal ambil data hero",Color3.fromRGB(255,80,80)); return
    end

    -- Sort heroes by Rank desc
    local heroList = {}
    ...
    local totalSlots = data.UnlockedHeroSlots or 10

    -- Cari hero yang sudah di-place
    local placed = {}
    local placedCount = 0
    for _, h in pairs(data.Heroes) do
        if h.PlaceInHand then
            placed[h.UniqueId] = true
            placedCount = placedCount + 1
        end
    end

    -- Ambil top N hero yang belum di-place
    local toPlace = {}
    for _, h in pairs(heroList) do
        if not placed[h.UniqueId] then
            table.insert(toPlace, h)
        end
        if #toPlace + placedCount >= totalSlots then break end
    end

    if #toPlace == 0 then
        setStatus("✅ Semua slot sudah terisi hero terbaik!",Color3.fromRGB(50,220,100)); return
    end

    setStatus("⭐ Equipping "..#toPlace.." hero terbaik...",Color3.fromRGB(255,215,0))
    for i, h in ipairs(toPlace) do
        pcall(function() PlaceHeroRemote:FireServer(h) end)
        task.wait(0.3)
        setStatus("⭐ Equip "..i.."/"..#toPlace..": "..h.HeroId.." Rank "..(h.Rank or 1),Color3.fromRGB(255,215,0))
    end
    setStatus("✅ "..#toPlace.." hero terbaik berhasil di-equip!",Color3.fromRGB(50,220,100))
end

equipOnceBtn.MouseButton1Click:Connect(function()
    equipOnceBtn.Active=false; equipOnceBtn.BackgroundColor3=Color3.fromRGB(100,80,0)
    doEquipBest()
    task.wait(0.5)
    equipOnceBtn.Active=true; equipOnceBtn.BackgroundColor3=Color3.fromRGB(160,120,0)
end)

makeSep(fusePage,8)

-- Auto equip toggle
local equipToggle=makeToggle(fusePage,"⭐  Auto Equip Best (tiap 30s)",false,9)
equipToggle.onChange(function(state)
    equipRunning=state
    if state then
        setStatus("⭐ Auto Equip Best ON",Color3.fromRGB(255,215,0))
        task.spawn(function()
        ...
        setStatus("✅ Auto Equip dimatikan",Color3.fromRGB(100,220,130))
    end
end)
...
-- Unequip all button
local unequipBtn=makeBtn(fusePage,"🗑️  Unequip Semua Hero",Color3.fromRGB(120,40,40),11)
unequipBtn.MouseButton1Click:Connect(function()
    if not GetDataRemote or not PickUpHeroRemote then return end
    local ok, data = pcall(function() return GetDataRemote:InvokeServer() end)
    if not ok or not data then return end
    local count = 0
    for _, h in pairs(data.Heroes) do
        if h.PlaceInHand then
            pcall(function() PickUpHeroRemote:FireServer(h) end)
            task.wait(0.2)
            count = count + 1
        end
    end
    setStatus("🗑️ "..count.." hero di-unequip",Color3.fromRGB(200,100,100))
end)
